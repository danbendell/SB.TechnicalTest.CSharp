...
namespace SB.TechnicalTest
{
    class Program
    {
        ...
        static int Attempt2()
        {
            var startingFloor = Building.NumberFloors / 2;
            return FindHighestSafeFloor(startingFloor, Building.NumberFloors);
        }

        static int FindHighestSafeFloor(int floor, int maxFloor)
        {
            var currentFloorPass = Building.DropMarble(floor);
            var nextFloorPass = Building.DropMarble(floor + 1);

            if (currentFloorPass && nextFloorPass)
            {
                //Go Up
                var nextFloor = floor + (maxFloor - floor) / 2;
                return FindHighestSafeFloor(nextFloor, maxFloor);
            };
            ...
            if (!currentFloorPass && !nextFloorPass)
            {
                //Go Down
                var nextFloor = floor - (maxFloor - floor) / 2;
                return FindHighestSafeFloor(nextFloor, floor);
            };
